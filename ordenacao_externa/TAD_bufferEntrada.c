...
{
  if(buffer->estado_buffer == 0) return NULL;
 
  if (Vazio_BufferEntrada(buffer) == 1){ // Caso a quantidade de registros consumidos seja igual a quantidade de registros contidos no buffer
  ...
  if(buffer->quantidade_consumidos_total == buffer->quantidade_registros_total) buffer->estado_buffer = 0; // Desativando o buffer
